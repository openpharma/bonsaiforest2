...
ENDPOINT_ID <- "binary" # <-- Set to "tte" for survival modeling
RESULTS_DIR <- "Results"

# Set unique seed based on script name for independent parallel execution
script_name <- "Binary_OVAT_1"
script_hash <- sum(utf8ToInt(script_name) * seq_along(utf8ToInt(script_name)))
unique_seed <- abs(as.integer((12345 + script_hash) %% .Machine$integer.max))
RNGkind('Mersenne-Twister')
set.seed(unique_seed)
cat(sprintf("Using unique seed %d for %s\n", unique_seed, script_name))
...
PRIOR_SPECIFICATIONS <- list(
  halfnormal = list(prior = NULL, stanvars = NULL)
)
...
flat_named_list <- flat_named_list[1:1000]
...
base_file_name <- paste(ENDPOINT_ID, "ovat_1", prior_names_str, sep = "_")
