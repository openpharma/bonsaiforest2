...
base_file_name <- paste(ENDPOINT_ID, "ovat_1", prior_names_str, "no_update", sep = "_")
...
# --- 6. Define the Parallel OVAT Function (FINAL CORRECTED VERSION) ---
  ...
  result_for_task <- tryCatch({
    ...
    for (covariate in subgr_vars) {
      ...
      tryCatch({
        ...
        # --- B. Fit from scratch ---
        # Note: We pass the whole 'prepared_run' object to 'prepared_model'
        ...
        summary_obj <- summary_subgroup_effects(
          fitted_model_cov, df, "arm", endpoint_params$resptype, subgroup_vars = 'auto'
        )
