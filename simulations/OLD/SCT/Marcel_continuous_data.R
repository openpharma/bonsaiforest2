#--------------------------------------------------------------------------------------------------
#------ SIMULATION TRUTH AND SIMULATION FUNCTION FOR THE CONTINUOUS SCENARIO
#------
#------ Select continuous scenarios from Sun et al (https://doi.org/10.1002/bimj.202100337) with interaction strengths b1_rel=0, 1, 2
#------ Re-scale parameter b0 to increase power for overall comparison from 50% to 90%
#------ Select only subgrouping variables X1, X2, X3, X4, X8, X11 (dichotomized at median), X14 (cut at Q1 and median), X17 (dichotomized at median)
...
selected_scen_par <- subset(scen_param, (type == "continuous") & (b1_rel %in% c(0,1,2)))
...
#------ re-scale b0 of selected_scen_par to ensure 90% (rather than 50%) power for group comparison and a sample size 250 per group
...
#---- Determine simulation truth for selected subgroups for all selected scenarios
...
for (i in (1:nrow(selected_scen_par))){
  ...
  true_subroup_effects_scenario <- df |>
    ...
    mutate(subgroup=fct_inorder(paste(subgroup_var,level,sep=".")),
  ...
  simulation_truth <- rbind(simulation_truth, true_subroup_effects_scenario)
}
...
ggplot(simulation_truth, aes(x = subgroup, y = trt_effect, color = subgroup_var)) +
  ...
  theme_bw() +
  theme(
    # Rotate subgroup labels vertically
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
    # Optional: Adds a subtle grid on the Y axis to help guide the eye
...
#---- Code to simulate a trial for a specific scenario_no

simulate_scenario <- function(scenario_no, selected_scen_par){
  scen <- selected_scen_par[scenario_no,]
  # Simulate covariates (via resampling from large population)
  df <-  benchtm:::X_large_pop[sample(1:50000, 500),]
  df$X11cat <- cut(df$X11, c(-Inf,0.4615385,Inf), labels = c("a", "b")) # cut at median
  df$X14cat <- cut(df$X14,c(-Inf,0.2478632, 0.3333833,Inf), labels = c("a", "b", "c")) # cut at Q1 and median
  df$X17cat <- cut(df$X17, c(-Inf,0.5888801 ,Inf), labels = c("a", "b")) # cut at median
...
sim_data <- simulate_scenario(scenario_no = 10, selected_scen_par = selected_scen_par)
