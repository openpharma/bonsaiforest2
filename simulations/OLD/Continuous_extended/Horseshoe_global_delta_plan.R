...
# Define parameters for horseshoe prior
sigma_plan <- 1.2
delta_plan <- 0.35
...
cat("Loading scenario data for scenarios 1-12...\n")
...
run_single_task <- function(i) {
  ...
  out <- tryCatch({
    ...
    est_df <- est_df %>% mutate(scenario_no = scenario_no, simulation_id = simulation_id, model_type = current_task$model_type, prior_name = current_task$prior_name, max_rhat = max_rhat, n_rhats_gt_1_05 = rhat_gt_1_05, n_divergent = n_divergent, mean_neff_ratio = neff_ratio)
