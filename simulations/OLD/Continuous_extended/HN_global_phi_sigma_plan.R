...
# Define sigma_plan = 1.2
sigma_plan <- 1.2

# Define the prior: Half-Normal with phi = sigma_plan
phi <- sigma_plan

message(paste("Defining Half-Normal prior with phi = sigma_plan =", phi, "..."))

stanvars_hn_phi_sigma <- brms::stanvar(
...
prior_hn_phi_sigma <- brms::set_prior("normal(0, sigma_pred)")

PRIOR_SPECIFICATIONS <- list(
  HN_global_phi_sigma_plan = list(prior = prior_hn_phi_sigma, stanvars = stanvars_hn_phi_sigma)
)
# ------------------------
