...
# === SCENARIOS 1-12 (CONSOLIDATED)
# === PARALLEL SIMULATION SCRIPT (OVAT + mclapply)
# === PRIOR: HN_phi_delta_plan (Half-Normal with phi = delta_plan)
# =========================================================================
...
PRIOR_SPECIFICATIONS <- list(
  HN_phi_delta_plan = list(
    ...
    prior = brms::set_prior(paste0("normal(0, ", delta_plan, ")"), class = "sd")
  )
)
...
# --- 3. LOAD AND PROCESS INPUT DATA (SCENARIOS 1-12) ---
...
cat(sprintf("Successfully loaded %d datasets from scenarios 1-12.\n", length(flat_named_list)))
