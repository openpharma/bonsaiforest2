...
library(bonsaiforest2); library(survival)
...
cat(sprintf("--- Starting GLOBAL SHRINKAGE MODEL RUN (%s) - SCENARIOS 1-12 ---\n", ENDPOINT_ID))
scenarios_to_run <- as.character(1:12)
scenarios_list <- list()
...
flat_named_list <- unlist(lapply(seq_along(scenarios_list), function(i) {
...
RNGkind('Mersenne-Twister'); set.seed(0)
...
base_file_name <- paste(ENDPOINT_ID, "global", paste(names(PRIOR_SPECIFICATIONS), collapse = "_"), sep = "_")
...
cat(sprintf("Starting %d total tasks...\n", total_tasks))
...
run_single_task <- function(i) {
  ...
  out <- tryCatch({
    ...
    cmdstan_output_dir <- file.path(job_temp_base, sprintf("cmdstan_%s_%s_%s", safe_prior_name, safe_sim_id, random_suffix))
    ...
    fit <- run_brms_analysis(
      data = df, response_formula = endpoint_params$resp_formula, response_type = endpoint_params$resptype,
      ...
      backend = "cmdstanr", output_dir = cmdstan_output_dir
    )
    ...
    # Parse sim_id into scenario_no and simulation_id
    id_parts <- strsplit(sim_id, "_")[[1]]
    scenario_no <- as.integer(id_parts[1])
    simulation_id <- as.integer(id_parts[2])

    est_df <- est_df %>% mutate(
      scenario_no = scenario_no, simulation_id = simulation_id,
      model_type = current_task$model_type, prior_name = current_task$prior_name,
      max_rhat = max_rhat, n_rhats_gt_1_05 = rhat_gt_1_05,
      n_divergent = n_divergent, mean_neff_ratio = neff_ratio
    )
  ...
  end_t <- Sys.time()
  cat(sprintf("[%s] DONE: %s (%.1f sec)\n", format(end_t, "%H:%M:%S"), task_desc, as.numeric(difftime(end_t, start_t, units = "secs"))), file = log_file, append = TRUE)
  out
}
...
cat(sprintf("Model run completed! Saved %d rows to %s\n", nrow(results_combined), results_file))
