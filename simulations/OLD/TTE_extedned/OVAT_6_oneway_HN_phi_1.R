# =========================================================================
# === ONE-WAY SHRINKAGE MODEL (OVAT) SCRIPT - SCENARIO 6
...
scenario_file <- file.path(scenarios_dir, "scenario6.rds")
scenarios <- list(readRDS(scenario_file))
all_datasets_processed <- lapply(seq_along(scenarios), function(i) {
  setNames(scenarios[[i]], paste(6, 1:length(scenarios[[i]]), sep = "_"))
})
...
base_file_name <- "tte_ovat_6_oneway_HN_phi_1"
...
if (nrow(final_results_df) > 0) {
  final_results_df <- final_results_df %>%
    ...
    mutate(scenario_id = 6, replication_id = as.integer(stringr::str_extract(sim_id, "\\d+$")), .before = 1) %>%
