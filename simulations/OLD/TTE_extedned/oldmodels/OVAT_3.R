...
# === 4. ADDED: Monkey-patch for bonsaiforest2::.calculate_bhaz_knots
# =========================================================================
...
# --- Define priors for random effects (Half-normal(1) on sd) ---
# Using random effects notation with default Half-normal(0, 1) priors on sd parameters
PRIOR_SPECIFICATIONS <- list(
  halfnormal_05 = list(prior = NULL, stanvars = NULL)
)
...
flat_named_list <- flat_named_list[2001:3000]
...
# --- 4. APPLY CRITICAL MONKEY PATCH (CORRECTED) ---

if (ENDPOINT_ID == "tte") {
  cat("\nApplying monkey-patch to fix knots for fast brms::update()...\n")
  ...
  # 3. Patch the function within the bonsaiforest2 NAMESPACE (not package environment)

  # Get the NAMESPACE environment (where internal functions live)
  bonsai_ns <- asNamespace("bonsaiforest2")

  # Store the original function so we can restore it later
  original_knots_fun <- get(".calculate_bhaz_knots", envir = bonsai_ns)

  # Apply the patch
  tryCatch({
    # Unlock binding in the namespace
    unlockBinding(".calculate_bhaz_knots", bonsai_ns)

    # Assign the fake function to the namespace
    assign(".calculate_bhaz_knots", fake_knots_fun, envir = bonsai_ns)

    # Lock binding again to protect it
    lockBinding(".calculate_bhaz_knots", bonsai_ns)

    cat("✓ Patch successfully applied to bonsaiforest2 namespace.\n")
  }, error = function(e) {
    cat("ERROR applying patch:", e$message, "\n")
    stop("Failed to patch .calculate_bhaz_knots")
  })

  # Ensure the original function is restored when this script exits
  on.exit({
    cat("Restoring original bonsaiforest2::.calculate_bhaz_knots function...\n")
    tryCatch({
      unlockBinding(".calculate_bhaz_knots", bonsai_ns)
      assign(".calculate_bhaz_knots", original_knots_fun, envir = bonsai_ns)
      lockBinding(".calculate_bhaz_knots", bonsai_ns)
    }, error = function(e) {
      cat("Warning: Could not restore original function:", e$message, "\n")
    })
  }, add = TRUE)

  cat("Knots fixed. Values:", paste(FIXED_KNOTS, collapse = ", "), "\n")
}
...
base_file_name <- paste(ENDPOINT_ID, "ovat_3", prior_names_str, sep = "_")
...
for (covariate in subgr_vars) {
  ...
  # 1. Prepare Data with random effects notation
  prepared_run <- bonsaiforest2::prepare_formula_model(
    ...
    unshrunk_terms_formula = paste("~", covariate, "+ arm"),
    shrunk_predictive_formula = paste("~ (0 + arm ||", covariate, ")")
  )

  # 2. Fit (Compiles here) - uses default Half-normal(1) priors on sd
  ...
  summ <- bonsaiforest2::summary_subgroup_effects(fit_obj)
  ...
  out <- tryCatch({
    ...
    for (covariate in subgr_vars) {
      ...
      updated_fit <- tryCatch({
        ...
        # If update fails (e.g., "CSV file corrupt"), try with recompile=TRUE as fallback
        if (grepl("corrupt|CSV", e$message, ignore.case = TRUE)) {
          cat(sprintf("  [%s] CSV error for %s, recompiling...\n", sim_id, covariate),
