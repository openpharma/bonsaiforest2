...
# ===
# === STRATEGY:
# === 1. Process only Scenario 4 data
...
PRIOR_SPECIFICATIONS <- list(
  ...
  HN_phi_delta_plan = list(
    ...
    prior = brms::set_prior("normal(0, 0.356)", class = "sd")
  ),
  HN_phi_delta_plan_half = list(
    name = "Half-Normal(delta_plan/2)",
    prior = brms::set_prior("normal(0, 0.178)", class = "sd")
  )
)
...
cat(sprintf("Successfully loaded %d datasets from scenario 4.\n", length(flat_named_list)))
...
# =========================================================================
# === PART B: RUN PARALLEL BY COVARIATE AND PRIOR
# =========================================================================
