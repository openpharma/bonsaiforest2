...
# === PRIOR: HN_phi_delta_plan_half (Half-Normal(delta_plan/2))
# ===
# === STRATEGY:
...
# =========================================================================
# === RUN FUNCTION FOR ONE-WAY MODELS (single covariate per task)
# =========================================================================
...
run_ovat_task <- function(i) {
  ...
  if (is.data.frame(out) && !"error" %in% names(out)) {
    ...
    out$task_idx <- i  # Store task index for proper mapping
...
# =========================================================================
# === PART B: RUN PARALLEL BY COVARIATE
# =========================================================================
# Process one covariate at a time across all datasets and the single prior
...
for (cov_idx in seq_along(subgr_vars)) {
  ...
  # If there are more tasks for this covariate, run them in parallel
  if (n_cov_tasks > 1) {
