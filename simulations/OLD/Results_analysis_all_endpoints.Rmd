...
base_path <- if (exists("params") && !is.null(params$base_path)) {
  params$base_path
} else {
  # Try to get from knitr, otherwise use current working directory
  if (requireNamespace("knitr", quietly = TRUE)) {
    knitr::current_input() %>% dirname()
  } else {
    getwd()
  }
}

# If base_path is NULL or doesn't contain "simulations", set it explicitly
if (is.null(base_path) || !grepl("simulations", base_path)) {
...
load_endpoint_data <- function(endpoint_id, ep_params) {
  ...
  if (!dir.exists(ep_params$folder)) {
    stop(paste("Endpoint folder does not exist:", ep_params$folder))
  }
  ...
  # Load truth data - check for both truth.RData and truth_2.RData
  ...
  if (!file.exists(truth_file)) {
    stop(paste("Truth file not found for endpoint:", ep_params$display_name))
  }
  ...
  standardized_results_list <- Filter(Negate(is.null), standardized_results_list)
  
  if (length(standardized_results_list) == 0) {
    stop(paste("No valid results files found for endpoint:", ep_params$display_name))
  }
...
plot_data_std <- all_standardized %>%
  mutate(scenario_name = factor(scenario_no, 
                               levels = names(scenario_labels), 
                               labels = scenario_labels))
plot_data_std <- plot_data_std%>%
  filter(estimator_name %in% c("population", "subgroup", "OVAT_hierarchical","OVAT_full_hierarchical", "Global_horseshoe_strong", "Global_full_hierarchical")) %>% 
  filter(endpoint=="Continuous Outcome")
plot_data_std[3,2] <- "OVAT_hierarchical"
