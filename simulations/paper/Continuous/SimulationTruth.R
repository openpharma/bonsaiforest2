...
#------ Select only subgrouping variables X1, X2, X4, X8, X11 (dichotomized at median),
...
#------ KEEP ONLY FIRST 3 SCENARIOS
selected_scen_par_power50 <- selected_scen_par_power50[1:3, ]

selected_scen_par_power50 <-
  selected_scen_par_power50 |>
  mutate(
    scenario_no = 1:3,
    scenario = fct_inorder(paste(rep(1, each = 3), case_match(b1_rel, 0 ~ "homo", 1~"hetero_mild", 2 ~"hetero_strong"), sep = "_"))
  ) |>
...
for (i in (1:nrow(selected_scen_par))){
  ...
  true_subgroup_effects_scenario <- df |>
    select(X1, X2, X4, X8, X11cat, X14cat, X17cat, trt_effect) |>
