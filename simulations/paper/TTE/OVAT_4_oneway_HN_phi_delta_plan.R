# =========================================================================
# === ONE-WAY SHRINKAGE MODEL (OVAT) SCRIPT - SCENARIO 4
# === PRIOR: HN_phi_delta_plan
# =========================================================================
...
scenario_file <- file.path(scenarios_dir, "scenario4.rds")
scenarios <- list(readRDS(scenario_file))
all_datasets_processed <- lapply(seq_along(scenarios), function(i) {
  setNames(scenarios[[i]], paste(4, 1:length(scenarios[[i]]), sep = "_"))
})
...
prior_names_str <- "HN_phi_delta_plan"
base_file_name <- paste(ENDPOINT_ID, "ovat_4_oneway", prior_names_str, sep = "_")
...
run_ovat_task <- function(i) {
  current_task <- task_grid[i, ]; sim_id <- current_task$sim_id; covariate <- current_task$covariate
  df <- flat_named_list[[sim_id]]; start_t <- Sys.time()
  ...
  out <- tryCatch({
    ...
    summ <- bonsaiforest2::summary_subgroup_effects(fit); est_df <- summ$estimates
    rhat_vals <- brms::rhat(fit); max_rhat <- ifelse(!is.null(rhat_vals), max(rhat_vals, na.rm = TRUE), NA)
    ...
    neff_ratio <- brms::neff_ratio(fit); mean_neff_ratio <- ifelse(!is.null(neff_ratio), mean(neff_ratio, na.rm = TRUE), NA)
    est_df$covariate_tested <- covariate; est_df$max_rhat <- max_rhat; est_df$n_rhats_gt_1_05 <- rhat_gt_1_05
    est_df$n_divergent <- n_divergent; est_df$mean_neff_ratio <- mean_neff_ratio; est_df
  }, error = function(e) {
    msg <- gsub("\\n", " ", e$message)
  ...
  if (is.data.frame(out) && !"error" %in% names(out)) {
    out$model_type <- current_task$model_type; out$prior_name <- current_task$prior_name; out$task_idx <- i
...
for (cov_idx in seq_along(subgr_vars)) {
  cov <- subgr_vars[cov_idx]
  idx <- which(task_grid$covariate == cov)
  if (length(idx) == 0) next
  cat(sprintf("\n--- Covariate %s (%d/%d) ---\n", cov, cov_idx, length(subgr_vars)))
  res1 <- run_ovat_task(idx[1])
  if (length(idx) > 1) {
    res_par <- mclapply(X = idx[-1], FUN = run_ovat_task, mc.cores = 96, mc.preschedule = FALSE)
    all_results_by_cov[[cov]] <- c(list(res1), res_par)
  } else {
    all_results_by_cov[[cov]] <- list(res1)
  }
}

final_results_df <- bind_rows(unlist(all_results_by_cov, recursive = FALSE))
if (nrow(final_results_df) > 0) {
  final_results_df <- final_results_df %>%
    left_join(task_grid %>% mutate(task_idx = row_number()) %>% select(task_idx, sim_id), by = "task_idx") %>%
    mutate(scenario_id = 4, replication_id = as.integer(stringr::str_extract(sim_id, "\\d+$")), .before = 1) %>%
