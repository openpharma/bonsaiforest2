# =========================================================================
# === ONE-WAY SHRINKAGE MODEL (OVAT) SCRIPT - SCENARIO 5
...
PRIOR_SPECIFICATIONS <- list(HN_phi_1 = list(name = "Half-Normal(1)", prior = brms::set_prior("normal(0, 1)", class = "sd")))
...
scenario_file <- file.path(scenarios_dir, "scenario5.rds")
scenarios <- list(readRDS(scenario_file))
all_datasets_processed <- lapply(seq_along(scenarios), function(i) {
  setNames(scenarios[[i]], paste(5, 1:length(scenarios[[i]]), sep = "_"))
})
...
base_file_name <- "tte_ovat_5_oneway_HN_phi_1"
...
run_ovat_task <- function(i) {
  ...
  }, error = function(e) { tibble(error = gsub("\\n", " ", e$message)) })
...
for (cov_idx in seq_along(subgr_vars)) {
  ...
  all_results[[cov]] <- if (length(idx) > 1) c(list(res1), mclapply(X = idx[-1], FUN = run_ovat_task, mc.cores = 96)) else list(res1)
}

final_results_df <- bind_rows(unlist(all_results, recursive = FALSE))
if (nrow(final_results_df) > 0) {
  final_results_df <- final_results_df %>%
    left_join(task_grid %>% mutate(task_idx = row_number()) %>% select(task_idx, sim_id), by = "task_idx") %>%
    mutate(scenario_id = 5, replication_id = as.integer(stringr::str_extract(sim_id, "\\d+$")), .before = 1) %>%
