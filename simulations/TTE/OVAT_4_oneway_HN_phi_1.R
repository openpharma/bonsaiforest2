...
for (cov_idx in seq_along(subgr_vars)) {
  ...
  cat(sprintf("\n--- Covariate %s (%d/%d) ---\n", current_cov, cov_idx, length(subgr_vars)))
  precompile_result <- run_ovat_task(cov_task_indices[1])
  if (n_cov_tasks > 1) {
    results_parallel <- mclapply(X = cov_task_indices[-1], FUN = run_ovat_task, mc.cores = num_cores, mc.preschedule = FALSE)
    cov_results <- c(list(precompile_result), results_parallel)
  } else {
    cov_results <- list(precompile_result)
  }
  all_results_by_cov[[paste(current_cov)]] <- cov_results
}
