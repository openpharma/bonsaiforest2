...
# Prior: Half-Normal with φ = δ_plan / 2
# Scenarios: 4-6
#
# This script:
...
# Define the prior: Half-Normal with phi = delta_plan / 2
phi <- delta_plan / 2

message(paste("Defining Half-Normal prior with phi = delta_plan/2 =", phi, "..."))
...
prior_hn_phi_delta_half <- brms::set_prior("normal(0, sigma_pred)")

PRIOR_SPECIFICATIONS <- list(
  HN_global_phi_delta_plan_half = list(prior = prior_hn_phi_delta_half, stanvars = stanvars_hn_phi_delta_half)
)
# ------------------------
...
cat(sprintf("--- Starting GLOBAL SHRINKAGE MODEL RUN (%s) - SCENARIOS 4-6 ---\n", ENDPOINT_ID))
...
base_file_name <- paste(ENDPOINT_ID, "global", prior_names_str, "scenarios_4_6", sep = "_")
