...
# Prior: Regularized Horseshoe with θ₀=δ_plan, s=2δ_plan
# Scenarios: 4-6
#
# This script:
...
# 3. Uses Regularized Horseshoe prior with scale_global=δ_plan, scale_slab=2δ_plan.
# 4. Processes SCENARIOS 4-6 only.
...
# Define the prior: Regularized Horseshoe with theta_0=delta_plan, s=2*delta_plan
theta_0 <- delta_plan
s <- 2 * delta_plan

message(paste("Defining Regularized Horseshoe prior with theta_0 = delta_plan =", theta_0, "and s = 2*delta_plan =", s, "..."))

PRIOR_SPECIFICATIONS <- list(
  RHS_theta0_delta_plan_s_2delta_plan = list(prior = paste0("horseshoe(df = 1, scale_global = ", theta_0, ", scale_slab = ", s, ")"), stanvars = NULL)
)
# ------------------------
...
cat(sprintf("--- Starting REGULARIZED HORSESHOE MODEL RUN (%s) - SCENARIOS 4-6 ---\n", ENDPOINT_ID))
cat("Loading scenario data for scenarios 4-6...\n")

scenarios_to_run <- as.character(4:6)
scenarios_list <- list()
...
all_datasets_processed <- lapply(scenarios_to_run, function(scen) {
  scenario_data <- scenarios_list[[scen]]
  setNames(scenario_data, paste(scen, 1:length(scenario_data), sep = "_"))
})
...
base_file_name <- paste(ENDPOINT_ID, prior_names_str, "scenarios_4_6", sep = "_")
