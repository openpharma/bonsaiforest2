...
for (scenario_no in 1:nrow(selected_scen_par)) {
  ...
  # Generate all simulations for this scenario
  ...
  # Add simulation IDs using bind_rows with .id
  names(scenario_data) <- 1:n_simulations
  scenario_data <- bind_rows(scenario_data, .id = "sim_id")
  scenario_data$sim_id <- as.integer(scenario_data$sim_id)
...
message(sprintf("Generated 12 scenarios with %d simulations each", n_simulations))
