...
#' @param original_data The original `data.frame` used for model fitting.
...
#'   Defaults to "auto". Cannot be NULL.
#'
#' @return An object of class `subgroup_summary`.
#'
...
summary_subgroup_effects <- function(brms_fit,
  ...
  # It must be "auto" OR a character vector.
  checkmate::assert(
  ...
  # (Previous "Overall" section removed)
...
plot.subgroup_summary <- function(x, x_lab = NULL, title = NULL, ...) {
  ...
  p <- ggplot(plot_data, aes(y = .data$y_axis_label, x = .data$Median)) +
    ...
    # Error bars
    # FIXED: Changed 'height' to 'width'. In geom_errorbar, 'width' controls the cap size
    # regardless of orientation.
    geom_errorbar(aes(xmin = .data$CI_Lower, xmax = .data$CI_Upper), width = 0.2, color = "black", orientation = "y") +
