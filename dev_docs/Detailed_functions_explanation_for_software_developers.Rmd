...
| 
| - Then calls: .ensure_numeric_binary_treatment(data, trt_var)
| - Validates: treatment is binary (2 levels if factor, 0/1 if numeric)
| - Converts: treatment to numeric 0/1 format
| - Purpose: Single point of validation eliminates repetitive checks
|
v
...
|
v
+-- 4) Apply contrast coding to variables (NEW UNIFIED APPROACH)
| - Calls: .apply_factor_contrasts_to_vars(data, var_names, user_contrast_vars, 
|                                          use_one_hot, calling_context)
| - Purpose: Unified function for applying contrasts (replaces duplicated logic)
| - For unshrunk terms: use_one_hot = FALSE (dummy coding, k-1 levels)
| - For shrunk terms: use_one_hot = TRUE (one-hot coding, k levels)
| - Respects user-specified contrasts (preserves if detected)
| - Converts character vectors to factors before applying contrasts
| - Eliminates ~105 lines of duplicated code across 3 previous functions
|
v
+-- 5) Detect and process predictive terms (create explicit interaction dummies)
...
|                                 shrunk_pred_formula, 
|                                 sub_formulas,
|                                 unshrunk_no_int)  # NEW: tracks user intercept preference
| - Key steps:
| * Builds sub-formulas as brms::lf() for unprogeffect, shprogeffect, unpredeffect, shpredeffect
| * Applies FLEXIBLE intercept rules (NEW BEHAVIOR):
|   - For survival models: ALWAYS force intercept removal (brms requirement)
|   - For other models: Respect user's intercept specification via unshrunk_no_int
|   - Ensures user control while maintaining model validity
