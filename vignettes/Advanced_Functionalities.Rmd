...
In `bonsaiforest2`, you can include the offset directly in the `response_formula` using the standard `brms` syntax: `outcome + offset(log_time) ~ trt`.
...
By default, `bonsaiforest2` uses one-hot encoding (all factor levels) for shrunk interaction terms, and dummy encoding (drop one level) for unshrunk terms. However, you can manually set custom contrasts in your data, and the library will respect your choice.

*Scenario*: You want to use treatment contrasts (k-1 dummy variables with a reference level) or custom contrasts (e.g., Helmert, sum coding) instead of one-hot encoding for your shrunk predictive effects.
