...
> **Pre-publication reference:** The statistical methodology implemented in this package is described in:
>
> Wolbers M\*, Pedrera Gómez M\*, Ocampo A, Gravestock I (2026). *Unified implementation and comparison of Bayesian shrinkage methods for treatment effect estimation in subgroups.* Data Science and Analytics, Roche, Basel, Switzerland. \*These authors contributed equally to this work.
...
The package enable the implementation of both a **global modeling approach** [@wolbers2025using], that estimates all prognostic and predictive effects in a single model, and a **One-variable-at a time** [@wang2024bayesian], that estimates the predictive effects using a different model for each subgrouping variable.
...
2.  **Advanced Shrinkage Priors:** Supports state-of-the-art shrinkage priors, including the **Regularized Horseshoe** [@piironen2017sparsity; @wolbers2025using] and **R2D2** [@zhang2022bayesian].
3.  **Marginal Effect Estimation:** Use of **standardization (G-computation)** to derive interpretable *marginal* subgroup treatment effects, which average over the covariate distributions within each subgroup [@HernanRobins2023]. This is implemented through:
...
This document is structured as follows: Section \@ref(sec-intro) provides a conceptual introduction to subgroup analysis challenges and the Bayesian shrinkage solution. Section \@ref(sec-methodology) details the statistical methodology, including notation, the global and the one-variable at a time models, endpoint-specific likelihoods, prior specifications, and the standardization procedure. Section \@ref(sec-functions) maps these statistical methods to the core functions in the `bonsaiforest2` package.
...
`bonsaiforest2` solves this by using **Standardization (G-computation)** [@HernanRobins2023]. This procedure correctly averages over the specific covariate distribution of each subgroup to estimate a valid marginal treatment effect, ensuring all subgroups are compared on the same interpretable scale. (See Section \@ref(sec-standardization) for a detailed explanation of how this standardization is performed in the package.)
...
-   $g(\cdot)$: A link function. In Section \@ref(sec-endpoint) we define the used link function for each type of endpoint.
...
-   **For shrunk terms** (prognostic or predictive): **One-hot encoding** is used, creating explicit dummy variables for *all* levels of each factor, with no reference level omitted. For example, a subgrouping variable with levels {A, B, C} creates three columns: `subgroupA`, `subgroupB`, `subgroupC` (for prognostic effects) or `trt:subgroupA`, `trt:subgroupB`, `trt:subgroupC` (for predictive effects). This approach ensures:

    -   All subgroup levels are treated **symmetrically** under the exchangeability assumption of the shrinkage prior
    ...
    -   Predictions work correctly through G-computation (see Section \@ref(sec-standardization))
...
-   $\mathbf{X}= [\mathbf{X_p} \;\;  \mathbf{X}_n]$ is the $N \times M$ design matrix for the **prognostic effects**. It typically includes:
    ...
    -   An intercept column. In Section \@ref(sec-intercept) we discuss the inclusion and the prior we give to the intercept in detail
    ...
    *Note:* For unshrunk terms, M=1+1+L-J= 1 for intercept + 1 for treatment + L (total subgroup levels) - J (number of subgrouping variables) because reference levels are dropped. For shrunk terms, all L levels are included (no reference level dropped).
...
-   $\mathbf{Z}= [\mathbf{Z_p} \;\;  \mathbf{Z}_n]$ is the design matrix for the **predictive effects**. The contrast coding depends on shrinkage status:
    ...
    -   **Shrunk predictive terms** ($\mathbf{Z}_p$): Use one-hot encoding where each column `trt_subgroupLEVEL` equals 1 when patient $i$ is both on treatment ($s_i = 1$) AND belongs to that specific subgroup level, and 0 otherwise. All $L$ interaction dummies are included to ensure all subgroups are treated symmetrically under the exchangeability assumption.
...
## One-Variable-at-a-Time Model {#sec-one-var}
...
While the intercept ($\beta_0$) is not subject to shrinkage (it is a fixed effect within the `unshrunktermeffect` component), it is important to understand how it is specified in the package.

**Default Specification:** The package uses `NULL` (brms default) for the intercept prior, allowing `brms` to automatically calibrate a weakly informative prior.

**User Customization:** Users can specify custom intercept priors via the `intercept_prior` parameter in `fit_brms_model()` or `run_brms_analysis()`. For example:

-   **For continuous outcomes:** `"normal(0, 10)"` provides a weakly informative prior centered at 0.
-   **For binary and count outcomes:** `"normal(0, 5)"` provides a prior on the log-odds or log-rate scale covering plausible effect sizes (e.g., ratios from approximately 0.08 to 12).
...
-   **Implementation:** The `brms` backend handles this automatically by estimating the baseline hazard (via `bhaz()`) rather than a fixed intercept parameter. Therefore, no intercept prior is applied for survival models.

**Additional Note:** The intercept prior is automatically omitted if the user specifies a formula without an intercept (using `~ 0 + ...` syntax).
...
## Prior Specifications {#sec-priors}

### Default Priors

When no user-specified priors are provided, the package applies the following automatic defaults:
...
-   **Shrunk prognostic:** `horseshoe(1)` — Strong regularization for main effects
-   **Shrunk predictive:** `horseshoe(1)` — Strong regularization for treatment interactions

**Special cases:**

-   **Random effects (one-way model, pipe-pipe `||` syntax):** SD parameters automatically receive `normal(0, 1)` priors on the standard deviation scale, regardless of the `shrunk_predictive_prior` setting.
-   **Survival models:** No intercept prior is applied, as Cox proportional hazards models do not have a global intercept; it is absorbed into the non-parametric baseline hazard.
-   **Formulas without intercepts:** If the user specifies a formula using `~ 0 + ...` syntax (no intercept), the intercept prior is omitted.

**User customization:** Users can override any default prior by specifying custom priors as character strings (e.g., `"normal(0, 2.5)"`) or as `brmsprior` objects via the parameters `intercept_prior`, `unshrunk_prior`, `shrunk_prognostic_prior`, and `shrunk_predictive_prior` in `fit_brms_model()` or `run_brms_analysis()`.

### Weakly Informative Priors (Unshrunk Terms)

For all unshrunk coefficients (both prognostic and predictive terms in the `unshrunktermeffect` component, as well as extra covariates $\boldsymbol{\beta_{3}}$), the package uses `brms` default weakly informative priors to aid computational stability without strongly influencing the posterior.

-   **Default:** `NULL` (uses `brms` defaults, typically `normal(0, ...)`). These priors are weakly regularizing on the linear predictor scale.
-   **User customization:** Users can override this by specifying custom priors (e.g., `"normal(0, 2.5)"`) via the `unshrunk_prior` parameter in `fit_brms_model()` or `run_brms_analysis()`.

### Regularized Horseshoe Prior (Shrunk Terms)

This is the default shrinkage prior in `bonsaiforest2`, recommended for its excellent adaptive shrinkage properties [@piironen2017sparsity].
...
-   **Hyperparameter Justification**: The package supports two ways to set the crucial global scale $\tau_0$:
    ...
    1.  **Fixed Default (`scale_global`):** `horseshoe(scale_global = 1)`. This is the package default, a robust, general-purpose choice.
...
## Estimation (MCMC) {#sec-estimation}
...
This section connects the statistical methodology (Section \@ref(sec-methodology)) to the package functions. The package provides a three-level architecture: high-level user interface functions, modular worker functions for advanced control, and internal helpers.
...
-   **`run_brms_analysis()`**
    ...
    -   **Maps to:** Sections \@ref(sec-global) (Global Model), \@ref(sec-one-var) (One-Variable-at-a-Time Model), \@ref(sec-endpoint) (Endpoints), \@ref(sec-priors) (Priors), \@ref(sec-estimation) (Estimation).
...
-   **`summary_subgroup_effects()`**
    ...
    -   **Maps to:** Section \@ref(sec-standardization) (Standardization / G-computation).
...
-   **`plot()`**
    ...
    -   **Maps to:** Final visualization of Section \@ref(sec-standardization) results.
...
-   **`prepare_formula_model()`**
    ...
    -   **Action:** Constructs and validates the three-component `brmsformula` structure:
        -   `unshrunktermeffect`: Unshrunk terms with weakly informative priors. Can include both prognostic effects (intercept, main treatment effect, pre-specified covariates) and predictive effects (treatment interactions). Specified via `unshrunk_terms_formula`.
...
-   **`fit_brms_model()`**
    ...
    -   **Action:** Executes the MCMC sampling and model fitting.
        ...
        -   Handles endpoint-specific configurations (Section \@ref(sec-endpoint)): likelihood functions, link functions, and stratification.
...
-   **`estimate_subgroup_effects()`**
    ...
    -   **Action:** Implements the complete G-computation procedure described in Section \@ref(sec-standardization):
