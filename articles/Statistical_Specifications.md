...
fixed effect within the `unshrunktermeffect` component), it is important
to understand how it is specified in the package.

**Default Specification:** The package uses `NULL` (brms default) for
the intercept prior, allowing `brms` to automatically calibrate a weakly
informative prior.

**User Customization:** Users can specify custom intercept priors via
the `intercept_prior` parameter in
...
- **For continuous outcomes:** `"normal(0, 10)"` provides a weakly
  informative prior centered at 0.
- **For binary and count outcomes:** `"normal(0, 5)"` provides a prior
  on the log-odds or log-rate scale covering plausible effect sizes
  (e.g., ratios from approximately 0.08 to 12).
...
- **Implementation:** The `brms` backend handles this automatically by
  ...
  intercept parameter. Therefore, no intercept prior is applied for
  survival models.

**Additional Note:** The intercept prior is automatically omitted if the
user specifies a formula without an intercept (using `~ 0 + ...`
syntax).
...
When no user-specified priors are provided, the package applies the
following automatic defaults:

**For all response types (continuous, binary, count, survival):**

- **Intercept:** `NULL` — Uses `brms` default prior (weakly informative)
- **Unshrunk terms:** `NULL` — Uses `brms` default priors (weakly
  informative)
- **Shrunk prognostic:** `horseshoe(1)` — Strong regularization for main
  effects
- **Shrunk predictive:** `horseshoe(1)` — Strong regularization for
  treatment interactions

**Special cases:**

- **Random effects (one-way model, pipe-pipe `||` syntax):** SD
  parameters automatically receive `normal(0, 1)` priors on the standard
  deviation scale, regardless of the `shrunk_predictive_prior` setting.
- **Survival models:** No intercept prior is applied, as Cox
  proportional hazards models do not have a global intercept; it is
  absorbed into the non-parametric baseline hazard.
- **Formulas without intercepts:** If the user specifies a formula using
  `~ 0 + ...` syntax (no intercept), the intercept prior is omitted.

**User customization:** Users can override any default prior by
specifying custom priors as character strings (e.g., `"normal(0, 2.5)"`)
or as `brmsprior` objects via the parameters `intercept_prior`,
`unshrunk_prior`, `shrunk_prognostic_prior`, and
`shrunk_predictive_prior` in
[`fit_brms_model()`](https://openpharma.github.io/bonsaiforest2/reference/fit_brms_model.md)
or
[`run_brms_analysis()`](https://openpharma.github.io/bonsaiforest2/reference/run_brms_analysis.md).
...
\\\boldsymbol{\beta\_{3}}\\), the package uses `brms` default weakly
informative priors to aid computational stability without strongly
influencing the posterior.

- **Default:** `NULL` (uses `brms` defaults, typically
  `normal(0, ...)`). These priors are weakly regularizing on the linear
  predictor scale.
- **User customization:** Users can override this by specifying custom
  priors (e.g., `"normal(0, 2.5)"`) via the `unshrunk_prior` parameter
  in
  [`fit_brms_model()`](https://openpharma.github.io/bonsaiforest2/reference/fit_brms_model.md)
  or
  [`run_brms_analysis()`](https://openpharma.github.io/bonsaiforest2/reference/run_brms_analysis.md).
